...
def _no_grad_trunc_normal_(tensor: torch.Tensor, mean: float, std: float, a: float, b: float) -> torch.Tensor:
    ...
    with torch.no_grad():
        ...
        lower: float = norm_cdf((a - mean) / std)
        upper: float = norm_cdf((b - mean) / std)
...
class ViTFeat(nn.Module):
    ...
    def __init__(
        self, pretrained_pth, feat_dim, vit_arch="base", vit_feat="k", patch_size=16
    ):
        super().__init__()
        if vit_arch == "base":
    ...
    def forward(self, img):
        ...
        with torch.no_grad():
            ...
            if self.vit_feat == "k":
                ...
                feats = (
                    q[:, 1:].transpose(1, 2).reshape(bs, self.feat_dim, feat_h * feat_w)
                )
            elif self.vit_feat == "v":
                feats = (
                    v[:, 1:].transpose(1, 2).reshape(bs, self.feat_dim, feat_h * feat_w)
                )
