...
class CustomSimpleTrainer(SimpleTrainer):
    ...
    def __init__(
        ...
        copy_paste_rate=-1,
        copy_paste_random_num=None,
        copy_paste_min_ratio=-1,
        copy_paste_max_ratio=-1,
        ...
        self.use_copy_paste = (
            use_copy_paste if self.cfg is None else self.cfg.DATALOADER.COPY_PASTE
        )
        self.cfg_COPY_PASTE_RATE = (
            copy_paste_rate if self.cfg is None else self.cfg.DATALOADER.COPY_PASTE_RATE
        )
        self.cfg_COPY_PASTE_RANDOM_NUM = (
            copy_paste_random_num
            if self.cfg is None
            else self.cfg.DATALOADER.COPY_PASTE_RANDOM_NUM
        )
        self.cfg_COPY_PASTE_MIN_RATIO = (
            copy_paste_min_ratio
            if self.cfg is None
            else self.cfg.DATALOADER.COPY_PASTE_MIN_RATIO
        )
        self.cfg_COPY_PASTE_MAX_RATIO = (
            copy_paste_max_ratio
            if self.cfg is None
            else self.cfg.DATALOADER.COPY_PASTE_MAX_RATIO
        )
        self.cfg_VISUALIZE_COPY_PASTE = (
            visualize_copy_paste
            if self.cfg is None
            else self.cfg.DATALOADER.VISUALIZE_COPY_PASTE
        )
    ...
    def copy_and_paste(self, labeled_data, unlabeled_data):
        ...
        for cur_labeled_data, cur_unlabeled_data in zip(labeled_data, unlabeled_data):
            ...
            if (
                self.cfg_COPY_PASTE_RATE >= copy_paste_rate
                and num_labeled_instances > 0
            ):
                if self.cfg_COPY_PASTE_RANDOM_NUM:
                    num_copy = (
                        1
                        if num_labeled_instances == 1
                        else np.random.randint(1, max(1, num_labeled_instances))
                    )
                else:
            ...
            if num_labeled_instances == 0 or num_copy == 0:
                ...
                choice = np.random.choice(
                    num_labeled_instances, num_copy, replace=False
                )
                copied_instances = cur_labeled_instances[choice].to(
                    device=cur_unlabeled_instances.gt_boxes.device
                )
                ...
                resize_ratio = random.uniform(
                    self.cfg_COPY_PASTE_MIN_RATIO, self.cfg_COPY_PASTE_MAX_RATIO
                )
                ...
                cur_labeled_image_new = (
                    F.interpolate(
                        cur_labeled_image[None, ...].float(),
                        size=(h_new, w_new),
                        mode="bilinear",
                        align_corners=False,
                    )
                ...
                if isinstance(copied_masks, torch.Tensor):
                    ...
                    masks_new = (
                        F.interpolate(
                            copied_masks.tensor[None, ...].float(),
                            size=(h_new, w_new),
                            mode="bilinear",
                            align_corners=False,
                        )
                ...
                copied_boxes.scale(
                    1.0 * unlabeled_w / labeled_w * resize_ratio,
                    1.0 * unlabeled_h / labeled_h * resize_ratio,
                )
                ...
                image_new_all[
                    :, h_shift : h_shift + h_new, w_shift : w_shift + w_new
                ] += cur_labeled_image_new
                masks_new_all[
                    :, h_shift : h_shift + h_new, w_shift : w_shift + w_new
                ] += masks_new
                ...
                if len(cur_unlabeled_instances) == 0:
                    ...
                    # merge image
                    alpha = alpha.cpu()
                    composited_image = (alpha * cur_labeled_image) + (
                        ~alpha * cur_unlabeled_image
                    )
                    ...
                    if isinstance(copied_masks, torch.Tensor):
                        iou_matrix = mask_iou_matrix(
                            copied_masks, cur_unlabeled_instances.gt_masks, mode="ioy"
                        )  # nxN
                    else:
                        iou_matrix = mask_iou_matrix(
                            copied_masks.tensor,
                            cur_unlabeled_instances.gt_masks.tensor,
                            mode="ioy",
                        )  # nxN
                    ...
                    if isinstance(copied_instances.gt_masks, torch.Tensor):
                        alpha = copied_instances.gt_masks.sum(0) > 0
                        cur_unlabeled_instances.gt_masks = (
                            ~alpha * cur_unlabeled_instances.gt_masks
                        )
                        areas_unlabeled = cur_unlabeled_instances.gt_masks.sum((1, 2))
                    else:
                        alpha = copied_instances.gt_masks.tensor.sum(0) > 0
                        cur_unlabeled_instances.gt_masks.tensor = (
                            ~alpha * cur_unlabeled_instances.gt_masks.tensor
                        )
                        areas_unlabeled = cur_unlabeled_instances.gt_masks.tensor.sum(
                            (1, 2)
                        )
                    # merge image
                    alpha = alpha.cpu()
                    ...
                    # merge instances
                    merged_instances = Instances.cat(
                        [cur_unlabeled_instances[areas_unlabeled > 0], copied_instances]
                    )
                    # update boxes
                    if isinstance(merged_instances.gt_masks, torch.Tensor):
                        merged_instances.gt_boxes = BitMasks(
                            merged_instances.gt_masks
                        ).get_bounding_boxes()
                        # merged_instances.gt_boxes = merged_instances.gt_masks.get_bounding_boxes()
                    else:
                        merged_instances.gt_boxes = (
                            merged_instances.gt_masks.get_bounding_boxes()
                        )
                ...
                if self.cfg_VISUALIZE_COPY_PASTE:
                    visualize_data(
                        cur_unlabeled_data,
                        save_path="sample_{}.jpg".format(np.random.randint(5)),
                    )
                new_unlabeled_data.append(cur_unlabeled_data)
...
class CustomAMPTrainer(CustomSimpleTrainer):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            copy_paste_rate=copy_paste_rate,
            copy_paste_random_num=copy_paste_random_num,
            copy_paste_min_ratio=copy_paste_min_ratio,
            copy_paste_max_ratio=copy_paste_max_ratio,
