...
def merged_clusters(i, j, clusters):
    ...
    weighted_sum = (
        (c1["feature"] + c2["feature"]) / (c1["num_of_patch"] + c2["num_of_patch"])
    ).float()
...
def iterative_merge(features, threshes, min_size=4):
    ...
    for y in range(H):
        for x in range(W):
            ...
            if (cluster_idx % W) != 0:
                ...
                similarities[(cluster_idx - 1, cluster_idx)] = torch.dot(
                    clusters[cluster_idx - 1]["normalized_feature"],
                    clusters[cluster_idx]["normalized_feature"],
                ).item()
            if (cluster_idx - W) >= 0:
                clusters[cluster_idx]["neighbors"].add(cluster_idx - W)
                clusters[cluster_idx - W]["neighbors"].add(cluster_idx)
                similarities[(cluster_idx - W, cluster_idx)] = torch.dot(
                    clusters[cluster_idx - W]["normalized_feature"],
                    clusters[cluster_idx]["normalized_feature"],
                ).item()
    ...
    for thresh in threshes:
        while len(similarities):
            ...
            for neighbor in merged["neighbors"]:
                if i in clusters[neighbor]["neighbors"]:
                    if neighbor < i:
                        del similarities[(neighbor, i)]
                    else:
                        del similarities[(i, neighbor)]
                    clusters[neighbor]["neighbors"].discard(i)
                if j in clusters[neighbor]["neighbors"]:
                    if neighbor < j:
                        del similarities[(neighbor, j)]
                    else:
                        del similarities[(j, neighbor)]
                    clusters[neighbor]["neighbors"].discard(j)

                similarities[(neighbor, cluster_idx)] = torch.dot(
        ...
        (
            all_masks.append(np.stack(single_level_masks))
            if len(single_level_masks)
            else None
        )
