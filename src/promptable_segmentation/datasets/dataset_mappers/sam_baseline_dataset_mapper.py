...
class SamBaselineDatasetMapper:
    ...
    def __init__(
        ...
        *,
        ...
        _root = os.getenv("TRAIN_DATASETS", "no")
        if _root != "no":
            totoal_images = 0
            if is_train:
            ...
            if self.is_train:
                assert (
                    not self.is_train
                ), "can not train without SA-1B datasets, please export"
    ...
    def __call__(self, idx_dict):
        ...
        if len(dataset_dict["annotations"]) == 0:
            print(
                "encounter image with empty annotations, choose the first image in the first tsv file"
            )
            current_tsv_id = 0
            current_idx = 0
            row = self.tsv[current_tsv_id].seek(current_idx)
        ...
        if "annotations" in dataset_dict:
            ...
            if (
                ...
                and "segmentation" in dataset_dict["annotations"][0].keys()
                and "size" in dataset_dict["annotations"][0]["segmentation"].keys()
            ):
            ...
            point_coords = []
            for ann in annos:
                # randomly select one from the list ann['point_coords']
                if "point_coords" in ann:
                    ann["point_coords"] = [random.choice(ann["point_coords"])]
            ...
            point_coords = torch.cat([point_coords - 3.0, point_coords + 3.0], dim=1)
            point_coords = transforms.apply_box(point_coords)
            point_coords = torch.as_tensor(
                point_coords, device=instances.gt_boxes.device
            )
        ...
        while (
            instances.gt_classes.shape[0] == 0
            or mask_shape[0] != ori_shape[0]
            or mask_shape[1] != ori_shape[1]
        ):
            print("current_tsv_id, current_idx", current_tsv_id, current_idx)
            (
                print(
                    "no masks! instances.gt_classes.shape[0] ",
                    instances.gt_classes.shape[0],
                )
                if instances.gt_classes.shape[0] == 0
                else print(
                    "wrong shape! mask_shape ", mask_shape, "ori_shape ", ori_shape
                )
            )
            ...
            if "annotations" in dataset_dict:
                ...
                point_coords = torch.cat(
                    [point_coords - 3.0, point_coords + 3.0], dim=1
                )
                point_coords = transforms.apply_box(point_coords)
                point_coords = torch.as_tensor(
                    point_coords, device=instances.gt_boxes.device
                ...
                dataset_dict["instances"] = instances
                print(
                    "generate a new index to solve this problem, now instances.gt_classes.shape[0]=",
                    instances.gt_classes.shape[0],
                )
