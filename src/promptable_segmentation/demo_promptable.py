...
def inference(image,text,text_part,text_thresh, *args, **kwargs):
    ...
    if device == 'cpu':
        semantic=False
        model=model_sam
        a,b= interactive_infer_image_idino_m2m(model, image,text,text_part,text_thresh,text_size,hole_scale,island_scale,semantic, device=device, *args, **kwargs)
        return a,b
    else:
