...
from semantic_sam.modules import (
    sem_seg_postprocess,
    SetCriterion,
    M2MHungarianMatcher,
...
class GeneralizedMaskDINO(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        weight_dict.update(
            {"loss_mask_bce_0": mask_weight, "loss_mask_dice_0": dice_weight}
        )
        ...
        criterion_mo1 = SetCriterion(
            ...
            semantic_ce_loss=dec_cfg["TEST"]["SEMANTIC_ON"]
            and dec_cfg["SEMANTIC_CE_LOSS"]
            and not dec_cfg["TEST"]["PANOPTIC_ON"],
            num_mask_tokens=dec_cfg.get("NUM_INTERACTIVE_TOKENS", 3),
        )
        ...
        sem_seg_head = build_semantic_sam_head(
    ...
    def evaluate_demo(
        ...
        targets[0]["points"] = (
            targets[0]["points"]
            * torch.as_tensor(
                [width, height, width, height], dtype=torch.float, device=self.device
            )
            / torch.as_tensor(
        ...
        outputs, mask_dict = self.sem_seg_head.predictor(
        ...
        mask_pred_results = outputs["pred_masks"].view(
            pred_ious.shape[0],
            pred_ious.shape[1],
            pred_ious.shape[2],
        ...
        mask_pred_results = torch.index_select(mask_pred_results, 2, level).flatten(
            1, 2
        )
    ...
    def forward(self, batched_inputs, inference_task="seg"):
        ...
        if self.training:
            ...
            if self.task_switch["sam"]:
                ...
                data = (
                    batched_inputs
                    if type(batched_inputs) == list
                    else batched_inputs["sam"]
                )
                losses_sam = self.forward_seg(
                    data, task="sam", prediction_switch=prediction_switch
                )
                new_losses_sam = {}
            ...
            if inference_task == "interactive":
                processed_results = self.evaluate_interactive(
                    batched_inputs,
                    task=inference_task,
                    prediction_switch=prediction_switch,
                )
            elif inference_task == "multi_granularity":
                processed_results = self.evaluate_interactive_granularity(
                    batched_inputs,
                    task=inference_task,
                    prediction_switch=prediction_switch,
                )
    ...
    def forward_seg(
        ...
        if self.training:
            ...
            if "instances" in batched_inputs[0]:
                ...
                targets = self.prepare_targets_interactive(
                    gt_instances, images, prediction_switch=prediction_switch
                )
            else:
                targets = None
            ...
            outputs, mask_dict = self.sem_seg_head(
                features, targets=targets, task=task, extra=prediction_switch
            )
            # bipartite matching-based loss
            losses = self.criterion(
                outputs, targets, mask_dict, task=task, extra=prediction_switch
            )
    ...
    def prepare_targets_interactive(
        self, targets, images, prediction_switch, task="seg"
    ):
        """
        ...
        box_start = random.randint(
            int((self.max_num_instance - 1) / 2), self.max_num_instance - 1
        )  # box based interactive after this number; about 1/4
        for targets_per_image in targets:
            gt_boxes = (
                targets_per_image.gt_boxes
                if torch.is_tensor(targets_per_image.gt_boxes)
                else targets_per_image.gt_boxes.tensor
            )
            # pad gt
            ...
            gt_masks = (
                targets_per_image.gt_masks
                if torch.is_tensor(targets_per_image.gt_masks)
                else targets_per_image.gt_masks.tensor
            )
            if not self.training:
            ...
            if len(gt_masks) == 0:
                new_targets.append(
                    {
                        "boxes": torch.ones(self.max_num_instance, 4)
                        .to(gt_masks)
                        .float(),
                        "points": torch.ones(self.max_num_instance, 4)
                        .to(gt_masks)
                        .float(),
                        "boxes_dn": torch.ones(self.max_num_instance, 4)
                        .to(gt_masks)
                        .float(),
                        "pb": torch.cat(
                            [
                                torch.ones(box_start),
                                torch.zeros(self.max_num_instance - box_start),
                            ],
                            0,
                        ),
                        "box_start": box_start,
            ...
            if num_mask == 0:
                print(
                    "wrong empty image! argets_per_image.gt_classes.shape[0] ",
                    targets_per_image.gt_classes.shape[0],
                    "targets_per_image",
                    targets_per_image,
                )
                ...
                for i in range(box_start):
                    ...
                    if not self.training and center_point:
                        ...
                        mask_dt = distance_transform(
                            (
                                ~F.pad(mask, pad=(1, 1, 1, 1), mode="constant", value=0)
                            ).float()
                        )[:, :, 1:-1, 1:-1]
                        # selected_index = torch.stack([torch.arange(n*_), mask_dt.max(dim=-1)[1].cpu()]).tolist()
                        selected_point = (
                            torch.tensor([mask_dt.argmax() / w, mask_dt.argmax() % w])
                            .long()
                            .cuda()
                            .flip(0)
                        )
                        ...
                        if (
                            ...
                            assert (
                                len(level_target_ind) > 0
                            ), "each point must have at least one target"
            ...
            new_target = {
                ...
                "labels": (
                    targets_per_image.gt_classes[index]
                ...
                "gt_whole_classes": (
                    ...
                    if targets_per_image.has("gt_whole_classes")
                    and prediction_switch["whole"]
                    else None
                ),
                "gt_part_classes": (
                    targets_per_image.gt_part_classes[index]
                    if targets_per_image.has("gt_part_classes")
                    and prediction_switch["part"]
                    else None
                ),
            }
            ...
            if not self.training:
                ...
                new_target["boxes_dn_ori"] = torch.cat(
                    [
                        new_target["points"].clone(),
                        new_target["boxes"][box_start:].clone(),
                    ],
                    0,
                )
                new_target["points"] = (
            ...
            new_target["boxes_dn"] = torch.cat(
                [new_target["points"], new_target["boxes"][box_start:]], 0
            )
    ...
    def evaluate_interactive(
        ...
        gt_instances = [x["instances"].to(self.device) for x in batched_inputs]
        targets_ = self.prepare_targets_interactive(
            gt_instances, images, prediction_switch=prediction_switch
        )
        outputs_, mask_dict = self.sem_seg_head(
            features, targets=targets_, task=task, extra=prediction_switch
        )
        outputs = {}
        targets = {}
        ...
        for key in outputs.keys():
            ...
            pred_masks_max = torch.gather(
                pred_masks.view(
                    -1, num_tokens, pred_masks.shape[-2], pred_masks.shape[-1]
                ),
                1,
                index[:, None, None, None].repeat(
    ...
    def evaluate_interactive_granularity(
        ...
        assert "instances" in batched_inputs[0]
        gt_instances = [x["instances"].to(self.device) for x in batched_inputs]
        targets_ = self.prepare_targets_sam_eval(
            gt_instances, images, prediction_switch=prediction_switch
        )
        outputs_, mask_dict = self.sem_seg_head(
            features, targets=targets_, task=task, extra=prediction_switch
        ...
        for key in outputs.keys():
            ...
            outputs_without_aux = {
                k: v for k, v in outputs[key].items() if k != "aux_outputs"
            }
            match_cost = ["cls", "box", "mask"]
            matcher = self.criterion_switch["m2m"].matcher
            criterion = self.criterion_switch["m2m"]
            indices = matcher(
                outputs_without_aux, targets[key], match_cost, extra=prediction_switch
            )
            ...
            ori_gt_masks = [
                torch.stack(
                    [ori_masks[ind] for inds in level_target_inds for ind in inds]
                )
            ]
