...
from semantic_sam.backbone.registry import register_backbone
...
class SwinTransformer(nn.Module):
    ...
    def load_weights(
        self, pretrained_dict=None, pretrained_layers=[], verbose=True, rename=False
    ):
...
class D2SwinTransformer(SwinTransformer, Backbone):
    ...
    def output_shape(self):
        feature_names = list(
            set(self._out_feature_strides.keys()) & set(self._out_features)
        )
        return {
            name: ShapeSpec(
...
def get_swin_backbone(cfg):
    ...
    if cfg["MODEL"]["BACKBONE"]["LOAD_PRETRAINED"] is True:
        ...
        print("Loading pretrained SwinT model from: ", filename)

        rename = True if "moby_swin" in filename else False
        ...
        swin.load_weights(
            ckpt,
            swin_cfg.get("PRETRAINED_LAYERS", ["*"]),
            cfg["VERBOSE"],
            rename=rename,
        )
        print("pretrained_layers", swin_cfg.get("PRETRAINED_LAYERS", ["*"]))
