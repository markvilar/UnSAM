...
from semantic_sam.body.decoder.registry import register_decoder
...
class IMaskDINODecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        trunc_normal_(self.lang_mapper, std=0.02)
        ...
        trunc_normal_(self.class_embed_part, std=0.02)
        ...
        self.num_all_tokens = (
            self.num_mask_tokens + self.iou_token
        )  # sam uses 4 to handle multi prompts
    ...
    def from_config(cls, cfg, in_channels, lang_encoder, mask_classification, extra):
        ...
        ret["initialize_box_type"] = dec_cfg[
            "INITIALIZE_BOX_TYPE"
        ]  # ['no', 'bitmask', 'mask2box']
        ret["dn"] = dec_cfg["DN"]
    ...
    def prepare_for_dn(self, targets, tgt, refpoint_emb, batch_size):
        ...
        if self.training:
            ...
            input_label_embed = (
                torch.gather(
                    self.lang_encoder.default_text_embeddings,
                    0,
                    m[:, None].repeat(1, self.dim_proj),
                )
                @ self.lang_mapper
            )
            ...
            if len(known_num):
                map_known_indice = torch.cat(
                    [input_label_embed.new_tensor(range(num)) for num in known_num]
                )  # [1,2, 1,2,3]
                map_known_indice = torch.cat(
    ...
    def prepare_for_dn_mo(self, targets, tgt, refpoint_emb, batch_size):
        ...
        ) + self.mask_tokens.weight.unsqueeze(0).repeat(
        ...
        for i in range(scalar):
            ...
            if i == scalar - 1:
                attn_mask[single_pad * i : single_pad * (i + 1), : single_pad * i] = (
                    True
                )
            else:
                attn_mask[
    ...
    def prepare_for_dn_mo_infer(self, targets, tgt, refpoint_emb, batch_size):
        ...
        m = known_labels_expaned.long().to(next(self.parameters()).device)
        m_pb = known_pb_labels_expaned.long().to(next(self.parameters()).device)
        input_label_embed = self.label_enc(m) + self.pb_embedding(m_pb)
        input_bbox_embed = inverse_sigmoid(known_bbox_expand)
    ...
    def forward(
        ...
        for i, output in enumerate(hs):
            outputs_class, outputs_mask, iou_score, decoder_output_mask = (
    ...
    def interactive_forward_prediction_heads(
        ...
        decoder_output = decoder_output + 0.0 * (
            self.class_embed_part.sum() + self.class_embed.sum()
        )

        out = decoder_output.view(
            decoder_output.shape[0], -1, self.num_all_tokens, decoder_output.shape[-1]
        )
        ...
        if self.prediction_switch["whole"]:
            ...
            outputs_class_whole = self.lang_encoder.compute_similarity(
                class_embed_whole, name="whole"
            )
        if self.prediction_switch["part"]:
            class_embed_part = decoder_output @ self.class_embed_part
            outputs_class_part = self.lang_encoder.compute_similarity(
                class_embed_part, name="part"
            )
        ...
        iou_score = (
            self.iou_prediction_head(decoder_output_iou)
            .squeeze(-1)
            .view(decoder_output.shape[0], -1, self.num_mask_tokens)
        )
    ...
    def _set_aux_loss(
        ...
        if out_boxes is None:
            ...
            return [
                ...
                for a, b, c in zip(
                    outputs_class[:-1], outputs_seg_masks[:-1], out_boxes[:-1]
                )
            ]
        else:
            return [
...
@register_decoder
def get_interactive_maskdino_transformer_decoder(
