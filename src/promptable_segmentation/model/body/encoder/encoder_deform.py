...
from semantic_sam.body.encoder.registry import register_encoder
from ...utils import configurable
from semantic_sam.modules import PositionEmbeddingSine
from semantic_sam.body.transformer_blocks import _get_clones, _get_activation_fn
...
class MaskDINOEncoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if self.transformer_num_feature_levels > 1:
            ...
            for _ in range(
                self.total_num_feature_levels - self.transformer_num_feature_levels
            ):  # exclude the res2
                input_proj_list.append(
                    nn.Sequential(
                        nn.Conv2d(
    ...
    def from_config(cls, cfg, input_shape: Dict[str, ShapeSpec], *args, **kwargs):
        ...
        ret["transformer_dim_feedforward"] = dec_cfg[
            "DIM_FEEDFORWARD"
    ...
    def forward_features(self, features, masks):
        ...
        if self.total_num_feature_levels > self.transformer_num_feature_levels:
            smallest_feat = features[
                self.transformer_in_features[self.low_resolution_index]
            ].float()
        ...
        y, spatial_shapes, level_start_index = self.transformer(
            srcs, masks, pos, use_ckpt=self.use_ckpt
        )
        bs = y.shape[0]
