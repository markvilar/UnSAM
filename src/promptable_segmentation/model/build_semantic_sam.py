...
def prepare_image(image_pth, img_size=640):
    """
    apply transformation to the image. crop the image ot 640 short edge by default
    """
    image = Image.open(image_pth).convert("RGB")
    t = []
    t.append(transforms.Resize(img_size, interpolation=Image.BICUBIC))
...
def build_semantic_sam(model_type, ckpt):
    ...
    model_semantic_sam = (
        BaseModel(opt, build_model(opt)).from_pretrained(ckpt).eval().cuda()
    )
