...
import numpy as np
import torch
from torchvision.ops.boxes import batched_nms, box_area, _batched_nms_vanilla, _batched_nms_coordinate_trick  # type: ignore
import torchvision
...
class SemanticSamAutomaticMaskGenerator:
    ...
    def _process_crop(
        ...
        #     keep_by_nms = _batched_nms_vanilla(data["boxes"].float(), data["iou_preds"], torch.zeros(len(data["boxes"])), self.box_nms_thresh)
        # else:
        #     keep_by_nms = _batched_nms_coordinate_trick(data["boxes"].float(), data["iou_preds"], torch.zeros(len(data["boxes"])), self.box_nms_thresh)
