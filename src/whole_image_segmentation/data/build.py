...
from mask2former import SamSelfTrainTSVDatasetMapper
...
def get_test_detection_dataset_dicts(
    ...
    """
    New get_detection_dataset_dicts modified by Authors of UnSAM
    only import the path to each json file and put them into a list
    """
    json_list = []
    image_path = MetadataCatalog.get(name).image_root

    for image_name in os.listdir(image_path):
        # extract image id from image name for all test datasets used in UnSAM, add more if needed
...
def get_tsv_train_detection_dataset_dicts():
    """
    return a tuple, first element is the tsv_id, second element is the lineidx
    """
    tsv_path = os.getenv("TRAIN_DATASETS", None)
    idx_list = []
...
def _train_loader_from_config(cfg, mapper=None, *, dataset=None, sampler=None):
    if dataset is None:
        dataset = get_tsv_train_detection_dataset_dicts()
