...
def sigmoid_ce_loss_droploss(
    ...
    if weights is not None:
        weights = weights.view(inputs.shape[0], 1)
        weights = weights.repeat(1, inputs.shape[-1])
        loss = loss * weights
    # 5.2614, 15.9631
...
class SetCriterion(nn.Module):
    ...
    def __init__(
        ...
        self.losses = losses
        self.use_drop_losses = use_drop_losses
    ...
    def forward(self, outputs, targets):
        ...
        # print(indices[0][0].shape)
        # a list of two tensor, each with shape 'num of gt'
